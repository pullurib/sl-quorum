...
async function main() {
  ...
  const tx1 = await b1Contract.proposeLoan(5n,20n,5n,0n);
  await tx1.wait();

  console.log(" borrower sent proposal ");
  ...
  const tx2 = await l1Contract.fundLoan(8n);
  await tx2.wait();
  console.log("lender funded proposal");

  const tx3 = await b1Contract.checkState(8n);
  await tx3.wait();

  console.log("Checked if borrower defaulted");
